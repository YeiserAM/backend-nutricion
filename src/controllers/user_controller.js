...
userCtr.getusuario = async (req, res) => {
	try {
		...
		const response = await pool.query(
			"select id_usuario,nombre , concat(apepat,' ',apemat) as Apellidos, codigo , dni from usuario u inner join persona p on u.idperson = p.idpersona  where id_usuario = $1"
		);
...
userCtr.singin = async (req, res) => {
	try {
		...
		const user = await pool.query("select * from usuario where usuario = $1 ", [
			usuario,
		]);
		...
		const sidebarResponse = await pool.query(
			"select * from sidebar where id_rol = $1",
			[response.rows[0].id_rol]
		);
...
(userCtr.autentication = verifyToken),
	(req, res) => {
...
userCtr.createUser = async (req, res) => {
	try {
		...
		const result = await pool.query(
			"insert into persona(nombre, apepat,apemat, dni, codigo ) values($1,$2,$3,$4,$5) returning *",
			[nombre, apepat, apemat, dni, codigo]
		);
		const password2 = await helpers.encryptPassword(password);
		await pool.query(
			"insert into usuario(usuario, password, id_rol,idperson) values($1,$2,$3,$4)",
			[usuario, password2, id_rol, result.rows[0].idperson]
		);
		return res.status(200).json(`Usuario ${usuario} creado correctamente...!`);
	} catch (e) {
		console.log(e);
...
userCtr.getAcceso = async (req, res) => {
	try {
		const id = parseInt(req.params.id);
		const resp = await pool.query("select * from accesos where id_rol =$1", [
			id,
		]);
